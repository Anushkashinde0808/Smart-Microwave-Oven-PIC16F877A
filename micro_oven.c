...
unsigned char pre_heat ;
 ...
 void display_power_screen(void)
 {
     ...
     {
         clcd_putch(BAR, LINE1(i));  //O 1 2 3.....15
         __delay_ms(100);
     }
     ...
     for(unsigned char i= 0; i < 16 ; i++)
     {
         clcd_putch(BAR, LINE4(1)); //0 1 2 3 ..... 15
         __delay_ms(100);
     }
     __delay_ms(1000);
 }
 ...
 void set_time(unsigned char key, unsigned char reset_flag)
 {
     ...
     if(key != '*' && key != '#' && key != ALL_RELEASED )//0 1 2 3 4 5 6 7
     {
         key_count++; //  <2 sec , >2 min
         if(key_count <= 3) // 2 times we are seconds
         {
             sec = sec * 10 + key;
             blink_pos = 0; // update blink        
     ...
     {
         delay = 0;
         display_blank = !display_blank;  //1, 0, 1, 0, 1, 0
        
     }
     //dispaly min and sec, blink sec field editable field
     if(display_blank)
     {
       if(blink_pos == 0)  
       {
     ...
     {
         clcd_putch(sec / 10 + '0', LINE2(8)); //30 30/10 3+'0' -> '3' ,  30%10-> 0+'0' -> '0'
 ...
 void set_temp( unsigned char key ,unsigned char  reset_flag)
 {
     ...
     if(key != '*' && key != '#' && key != ALL_RELEASED )//0 1 2 3 4 5 6 7
     {
         ...
         {
             temp = temp * 10 + key; // 1*10 // 10 * !0 -> 100+2
         }
     }
     //* cler time
     else if( key == '*')
     ...
     else if(key == '#')       
     {
         ...
         while(pre_heat)
         {
             clcd_putch(pre_heat / 100 + '0', LINE3(9)); //300-> 300/100 30/10  3+ '0' -> '3' ,  30%10-> 0+'0' -> '0'
     ...
     if(display_blank)   // to blink temperature
     {
        clcd_putch(' ', LINE2(7));
        clcd_putch(' ', LINE2(8));
        clcd_putch(' ', LINE2(9)); 
     ...
     {
        clcd_putch(temp / 100 + '0', LINE2(7)); //300-> 300/100 30/10  3+ '0' -> '3' ,  30%10-> 0+'0' -> '0'
 ...
 void display_time(void)
 {
     ...
     clcd_putch(':', LINE1(11));
     clcd_putch( sec / 10 + '0', LINE1(12)); //30 30/10 3+'0' -> '3' ,  30%10-> 0+'0' -> '0'
